 ...
 */
static bool setupLarod(const char* chipString, const char* modelFile,
                       larodConnection** larodConn, larodModel** model)
{
    ...
    int larodModelFd = open(modelFile, O_RDONLY);
    if (larodModelFd < 0) {
...
int main(int argc, char** argv)
{
    ...
    ppMap = CreatePreProcessMap(streamWidth, streamHeight, inputWidth, inputHeight);
    cropMap = CreateCropMap(clipX, clipY, clipW, clipH);

    // Create larod models
    syslog(LOG_INFO, "Setting up larod connection with chip %s and model file %s", chipString, modelFile);
    if (!setupLarod(chipString, modelFile, &conn, &model)) {
        goto end;
    }
